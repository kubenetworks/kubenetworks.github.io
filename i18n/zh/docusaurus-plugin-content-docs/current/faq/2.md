...
# 使用 WSL( Windows Sub Linux ) Docker, 用命令 `kubevpn run` 进入运行模式的时候, 在 terminal 中无法提示链接集群网络, 这是为什么, 如何解决?
...
- 2): 在主机 Windows 使用命令 `kubevpn connect`, 然后在 WSL 中使用 `kubevpn run` 进入运行模式
- 3): 在主机 Windows 上启动一个 container，在 container 中使用命令 `kubevpn connect`, 然后在 WSL
  中使用 `kubevpn run --network container:$CONTAINER_ID`
